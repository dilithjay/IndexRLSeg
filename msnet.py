# Reference: https://github.com/taochx/MSNet/blob/main/msnet.py
...
class nnn_net(nn.Module):
    def __init__(self, num_classes, in_channels, filters=32):
        super().__init__()
        self.nnn = resnet50(pretrained=True)

        if in_channels != 3:
            self.nnn.conv1 = nn.Conv2d(
                1, 64, kernel_size=7, stride=2, padding=3, bias=False
            )
            self.nnn.conv1.weight.data = torch.unsqueeze(
                torch.mean(self.nnn.conv1.weight.data, dim=1), dim=1
            )
...
class MSNet(nn.Module):
    def __init__(self, rgb_channels, nnn_channels, num_classes):
        super(MSNet, self).__init__()
        self.rgb = rgb_net(num_classes)
        self.nnn = nnn_net(num_classes, len(nnn_channels))

        self.rgb_channels = rgb_channels
        self.nnn_channels = nnn_channels
    ...
    def forward(self, rgbnnd):
        ...
        rgb = rgbnnd[:, self.rgb_channels]
        nnn = rgbnnd[:, self.nnn_channels]
