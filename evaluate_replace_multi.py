...
model_dir = "models/5-replace_multi"
paths = glob(os.path.join(model_dir, "*.ckpt"))
names = []
for path in paths:
    name = os.path.basename(path).split(".")[0]
    names.append(name)
    arch, dataset, channels = name.split("_")
    ...
    from model import SegModelReplace
    import os
    import random
    ...
    class CustomDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            max_z = 3
            idx1 = eval_expression(self.exp[0], img)
            idx1 = (idx1 - idx1.mean()) / idx1.std()
            idx1 = (np.clip(idx1, -max_z, max_z) + max_z) / (2 * max_z)

            idx2 = eval_expression(self.exp[1], img)
            idx2 = (idx2 - idx2.mean()) / idx2.std()
            idx2 = (np.clip(idx2, -max_z, max_z) + max_z) / (2 * max_z)

            img[channels.index("1")] = idx1
            img[channels.index("2")] = idx2
